...
public abstract class PrepareFeatureUtil extends ServerFeatureUtil {
    ...
    private void prepareFeature(String groupId, String artifactId, String version) throws PluginExecutionException {
    	...
    	try {
    		...
    		debug(e);
        	info(String.format("Cannot find json for coordinate %s:%s:%s in connected repositories", groupId, artifactId, version));
    	}
    	if (json != null) {
        	...
        	try {
            	...
            	String targetJsonFile = createArtifactFilePath(repoLocation, groupId, FEATURES_JSON_ARTIFACT_ID, "json", version);
            	...
            	debug("expecting BOM file artifact to be in repository: " + e.getMessage());
        	}
    	}
    }
     ...
     * @throws PluginExecutionException throws error if unable to download the artifacts
     */
     ...
     * @return The formatted filepath for the given artifact in the repoLocation repository
     */
    ...
    /**
     * Parse the repository location when given a path to an artifact in the repository
     * 
    ...
    private String parseRepositoryLocation(File fileFromRepo, String groupId, String artifactId, String fileType, String version) {
        ...
        groupId = groupId.replace(".", "/");
        String fileSubString = String.format("%s/%s/%s/%s-%s.%s", groupId, artifactId, version, artifactId, version, fileType);

        return absFileFromRepo.replace(fileSubString, "");
    }

    /**
     * Generate JSON at targetJsonFile location for the list of ESAs in esaFiles under the given groupId
     * 
    ...
    public File generateJson(String targetJsonFile, Map<File, String> esaFileMap) throws PluginExecutionException {
        try {
            ...
            throw new PluginExecutionException(
                    "Cannot read or create json file " + targetJsonFile, e);
        }
    }
	...
	private File downloadOverrideJar(String groupId, String artifactId) {
        try {
            return downloadArtifact(groupId, artifactId, "jar",
                    String.format("[%s)", openLibertyVersion + ", " + InstallFeatureUtil.getNextProductVersion(openLibertyVersion)));
        } catch (PluginExecutionException e) {
