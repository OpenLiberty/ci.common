...
public class DevUtilTest extends BaseDevUtilTest {
    ...
    @Test
    public void testIsNotGeneratedTargetFile() throws Exception {
        File tempTextFileSource = new File(configDirectory, "temp1.txt");
        File tempTextFileTarget = new File(targetDir, "temp1.txt");
        Files.write(tempTextFileTarget.toPath(), "ABC=123\nDEF=456".getBytes());
        assertTrue(tempTextFileTarget.exists());

        assertFalse("File should not be detected as a generated file", util.isGeneratedTargetFile(tempTextFileSource, configDirectory, targetDir));
    }

    @Test
    public void testIsGeneratedTargetFile() throws Exception {
        File tempTextFileSource = new File(configDirectory, "temp2.txt");
        File tempTextFileTarget = new File(targetDir, "temp2.txt");
        Files.write(tempTextFileTarget.toPath(), "# Generated by liberty-maven-plugin\nABC=123".getBytes());
        assertTrue(tempTextFileTarget.exists());

        assertTrue("File should be detected as a generated file", util.isGeneratedTargetFile(tempTextFileSource, configDirectory, targetDir));
    }
